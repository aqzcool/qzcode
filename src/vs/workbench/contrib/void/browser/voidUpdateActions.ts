...
const notifyUpdate = (res: VoidCheckUpdateRespose & { message: string }, notifService: INotificationService, updateService: IUpdateService): INotificationHandle => {
	const message = res?.message || 'This is a very old version of QZ, please download the latest version! [QZ Editor](https://qz.cool/download-beta)!'
		...
		if (res.action === 'reinstall') {
			primary.push({
				...
				run: () => {
					...
					window.open('https://qz.cool/download-beta')
				}
			})
		}
...
const notifyErrChecking = (notifService: INotificationService): INotificationHandle => {
	const message = `QZ Error: There was an error checking for updates. If this persists, please get in touch or reinstall QZ [here](https://qz.cool/download-beta)!`
	const notifController = notifService.notify({
		severity: Severity.Info,
		message: message,
		sticky: true,
